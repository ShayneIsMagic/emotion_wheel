...
export class MarkdownToPDFConverter {
   ...
   */
  convertMarkdownToPDF(markdownContent: string, title: string, subtitle: string): jsPDF {
    this.addHeader(title, subtitle);

    // Parse markdown and convert to PDF sections
   ...
   */
  private parseMarkdown(markdown: string): Array<{title: string, content: string[]}> {
    ...
    lines.forEach(line => {
      ...
      if (trimmedLine.startsWith('## ')) {
        ...
        const cleanLine = this.cleanMarkdownLine(trimmedLine);
        if (cleanLine) {
  ...
  /**
   * Generate PDF from the integrated personality assessment markdown
   */
  generateIntegratedPersonalityPDF(): jsPDF {
    const markdownContent = `# Integrated Personality Assessment
...
## CliftonStrengths Emotional Patterns
...
• Communication effectiveness improvement`;
    ...
    return this.convertMarkdownToPDF(markdownContent, 'Integrated Personality Assessment', 'CliftonStrengths + HEXACO Integration');
  }

  /**
   * Generate PDF from the clinical emotional assessment markdown
   */
  generateClinicalEmotionalPDF(): jsPDF {
    const markdownContent = `# Clinical Emotional Assessment
...
## Emotion Inventory - Trust Family
### Level 1 - Acceptance
...
• Assessing therapeutic intervention effectiveness`;
    ...
    return this.convertMarkdownToPDF(markdownContent, 'Clinical Emotional Assessment', 'For Licensed Mental Health Professionals');
  }

  /**
   * Generate PDF from the unfiltered scoring assessment markdown
   */
  generateUnfilteredScoringPDF(): jsPDF {
    const markdownContent = `# Unfiltered Scoring Assessment
...
## Score Interpretation & Implications
...
• Mathematical validation of emotional patterns`;
    ...
    return this.convertMarkdownToPDF(markdownContent, 'Unfiltered Scoring Assessment', 'Raw Mathematical Scoring Framework');
  }
